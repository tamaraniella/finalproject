...
ggplot(summary2, aes(story, prop_agreement))+
  ...
  geom_errorbar(aes(ymin = prop_agreement + qnorm(0.025)*se_agree,
                    ...
                    group = Mindset), size=.3, width= .2, position=position_dodge(0.9)) +
  scale_fill_manual(values=wes_palette(name="Moonrise2", n=2))+
  labs(title= "Proportion of agreement in Control vs Doubt, by Story", x = "Story", y = "Proportion of agreement")+
...
#plot 2 - version without dividing by stories #NEW OK VERSION with error bars AND percentage instead of proportion 

ggplot(summary_cond_se2, aes(condition, per_agreement))+
  geom_col(alpha=.7, fill = c("#F1BB7B", "#FD6467"),width = .5)+
  geom_errorbar(aes(ymin = per_agreement + qnorm(0.025)*se_agree,
                    ymax = per_agreement + qnorm(0.975)*se_agree,
                    group = condition), size=.3, width= .2) +
  labs(title= "Expected Agreement in Imagine vs Agreement in Interaction", x = "Condition", y = "Percentage of agreement")+
  theme_minimal(base_size = 15)
...
ggplot(tn_dat, aes(part, score)) +
  geom_violin(color = "#F97FF5") +
  facet_wrap(~mindset) +
  theme_minimal()

ggplot(tn_dat, aes(part, score)) +
  geom_density_ridges(group= mindset) +
  #facet_wrap(~mindset) +
  theme_minimal()
...
#adding colors #I THINK I AM GOING TO KEEP THIS ONE
ggplot(tn_dat, aes(score,mindset))+
  geom_density_ridges(aes(fill=mindset, color= mindset), alpha=.7)+
  scale_fill_manual(values=wes_palette(name="Cavalcanti1", n=2))+
  scale_color_manual(values=wes_palette(name="Cavalcanti1", n=2))+
  facet_wrap(~part) +
  theme_minimal(base_size = 15)

#trying out Viridis palette
ggplot(tn_dat, aes(score,mindset))+
  geom_density_ridges(aes(fill=mindset, color= mindset), alpha=.7)+
...
ggplot(tn_dat, aes(score,mindset))+
  ...
  facet_wrap(~part) +
  labs(title= "Distribution of Judgment before (Part 1) and after (Part 3) discussion with partner", x = "Judgment", y = "Density")+
  theme_minimal(base_size = 15)


#Another try for plot 3: 
ggplot(tn_dat, aes(score, color = part, fill = part))+
  geom_density(alpha = 0.4)+
  scale_fill_manual(values=wes_palette(name="Moonrise2", n=2))+
  scale_color_manual(values=wes_palette(name="Moonrise2", n=2))+
  facet_wrap(~mindset, ncol=1) +
  theme_minimal(base_size = 15)
